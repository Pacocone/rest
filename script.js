...
function initForm(){
  ...
  // Escucha múltiples tipos de eventos en los campos de comensales e importe total.
  // Añadimos eventos adicionales como 'keypress', 'mousedown', 'touchstart' y 'pointerdown' para mejorar la compatibilidad en móviles.
  ...
  setStep('bindStars');
  // Para manejar mejor eventos táctiles y de ratón en la selección de estrellas, escuchamos varios tipos de eventos.
  ['click','touchstart','pointerdown','mousedown'].forEach(function(ev){
    starsEl.addEventListener(ev, function(e){
      // Cancelamos la propagación y comportamiento por defecto para evitar duplicidad de eventos.
  ...
  ['click','touchstart','pointerdown','mousedown'].forEach(function(ev){
    ...
    rc.addEventListener(ev, function(e){
      if(e && e.preventDefault) e.preventDefault();
      hidden.value='0';
      paint(0);
      setStep('star:0');
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  // Aseguramos que el envío de enlace mágico sólo se pida una vez. Si ya se envió previamente, desactivamos el formulario.
  try {
    var emailInput = byId('authEmail');
    var magicBtn = byId('sendMagicLink');
    var wasReg = false;
    try { wasReg = localStorage.getItem('rt_is_registered') === 'true'; } catch(_e) { wasReg = false; }
    if(wasReg) {

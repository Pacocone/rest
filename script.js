...
/* === Utils === */
var DB_KEY='visitas_restaurantes_v1361';
...
function initTheme(){
  ...
  if(pref==='system'){
    ...
    applyTheme(d?'dark':'light'); try{localStorage.setItem('rt_theme','system');}catch(e){}
  }else{ applyTheme(pref); }
  var dEl=byId('chipDark'),lEl=byId('chipLight'),sEl=byId('chipSystem');
...
function handleAttInput(files){
  ...
  (function next(i){
    if(i>=files.length){renderAttPreview();return;}
    var f=files[i]; var key='a_'+uid();
    attPut(key,f).then(function(){stagedAttKeys.push(key);next(i+1);});
  })(0);
}
function renderAttPreview(){
  var wrap=byId('attPreview'); if(!wrap)return; wrap.innerHTML='';
  var i=0; (function addNext(){
    if(i>=stagedAttKeys.length)return;
    var key=stagedAttKeys[i++];
    attGet(key).then(function(rec){
      if(!rec){addNext();return;}
      var url=URL.createObjectURL(rec.blob);
      var el=document.createElement('div'); el.className='att';
      if((rec.type||'').indexOf('image/')===0){
        var img=new Image(); img.src=url; img.alt='Adjunto'; img.loading='lazy'; el.appendChild(img);
      }else{
        var box=document.createElement('div'); box.style.cssText='width:100%;height:80px;display:flex;align-items:center;justify-content:center;color:#fff;'; box.textContent='Archivo'; el.appendChild(box);
      }
      ...
      var rm=document.createElement('button'); rm.className='remove'; rm.type='button'; rm.textContent='×';
      rm.addEventListener('click',function(){attDelete(key).then(function(){stagedAttKeys=stagedAttKeys.filter(function(k){return k!==key;});renderAttPreview();});});
      el.appendChild(rm);
...
function initTabs(){
  ...
  for(var i=0;i<tabs.length;i++){(function(btn){
    btn.addEventListener('click',function(){
      ...
      byId('tab-'+btn.getAttribute('data-tab')).classList.add('active');
    });
  })(tabs[i]);}
}
...
function initForm(){
  ...
  function paint(v){var bs=starsEl.querySelectorAll('button[data-value]');for(var i=0;i<bs.length;i++){var val=parseInt(bs[i].getAttribute('data-value'),10);if(val<=v)bs[i].classList.add('filled');else bs[i].classList.remove('filled');}}
  var btns=starsEl.querySelectorAll('button[data-value]'); for(var i=0;i<btns.length;i++){(function(b){b.addEventListener('click',function(){hidden.value=b.getAttribute('data-value');paint(parseInt(hidden.value,10));});})(btns[i]);}
  byId('ratingClear').addEventListener('click',function(){hidden.value='0';paint(0);});
  byId('visitForm').addEventListener('submit',function(e){
    ...
    if(editingId){
      ...
      if(v){v.restaurant=restaurant;v.city=city;v.date=date;v.diners=diners;v.total=total;v.avg=diners?total/diners:0;v.rating=rating;v.notes=notes;v.mapsUrl=mapsUrl||'';v.attKeys=stagedAttKeys.slice();v.updatedAt=Date.now();}
      state.visits.sort(function(a,b){return (b.date||'').localeCompare(a.date||'');});
    }else{
      var visit={id:uid(),restaurant:restaurant,city:city,date:date,diners:diners,total:total,avg:diners?total/diners:0,rating:rating,notes:notes,mapsUrl:mapsUrl||'',attKeys:stagedAttKeys.slice(),updatedAt:Date.now()};
...
function startEdit(id){
  stagedAttKeys=[];
  var v=null; for(var i=0;i<state.visits.length;i++){if(state.visits[i].id===id){v=state.visits[i];break;}}
  if(!v)return;
  ...
  var diners=parseInt(byId('diners').value||'0',10); var total=parseFloat(byId('total').value||'0'); byId('avgPerDiner').value=diners>0?fmt2(total/diners)+' €':'—';
  var bs=byId('rating').querySelectorAll('button[data-value]'); for(var i=0;i<bs.length;i++){var val=parseInt(bs[i].getAttribute('data-value'),10); if(val<=v.rating)bs[i].classList.add('filled'); else bs[i].classList.remove('filled');}
...
function cancelEdit(){
  editingId=null; byId('submitBtn').textContent='Guardar visita'; byId('cancelEdit').style.display='none'; byId('visitForm').classList.remove('editing');
  byId('visitForm').reset(); byId('date').value=todayISO(); byId('ratingValue').value='0'; 
  var bs=byId('rating').querySelectorAll('button[data-value]'); for(var i=0;i<bs.length;i++){bs[i].classList.remove('filled');}
  byId('avgPerDiner').value='—'; stagedAttKeys=[]; renderAttPreview();
}
...
function initDataOps(){
  byId('exportBtn').addEventListener('click',function(){
    ...
    var url=URL.createObjectURL(blob); var a=document.createElement('a'); a.href=url; a.download='mis-restaurantes.json'; a.click(); URL.revokeObjectURL(url);
  });
  byId('importInput').addEventListener('change',function(e){
    var file=e.target.files&&e.target.files[0]; if(!file)return; var reader=new FileReader();
    reader.onload=function(){
      try{
        var arr=JSON.parse(reader.result); if(!Array.isArray(arr))throw new Error('Formato no válido');
        arr=arr.map(function(v){return{id:v.id||uid(),restaurant:String(v.restaurant||'').slice(0,200),city:String(v.city||''),date:v.date,diners:Math.max(1,parseInt(v.diners,10)||1),total:Math.max(0,parseFloat(v.total)||0),avg:Math.max(0,parseFloat(v.avg)||0),rating:Math.max(0,Math.min(5,parseInt(v.rating,10)||0)),notes:String(v.notes||'').slice(0,2000),mapsUrl:String(v.mapsUrl||''),attKeys:Array.isArray(v.attKeys)?v.attKeys:[],updatedAt:Date.now()};});
        state.visits=arr.sort(function(a,b){return (b.date||'').localeCompare(a.date||'');}); save(); renderAll(); alert('Datos importados.');
...
function renderRecent(){
  ...
  for(var i=0;i<Math.min(8,state.visits.length);i++){
    var v=state.visits[i];
    var li=document.createElement('li');
    var left=document.createElement('div'); var right=document.createElement('div');
    var name=document.createElement('div'); name.textContent=v.restaurant;
    var meta=document.createElement('div'); meta.className='muted';
    var avgText=v.diners?fmtEUR(v.total/v.diners):'—'; var cityTxt=v.city?' • 📍 '+v.city:'';
    ...
    var eb=document.createElement('button'); eb.className='btn secondary'; eb.textContent='Editar'; (function(id){eb.addEventListener('click',function(){startEdit(id);});})(v.id); right.appendChild(eb);
    var db=document.createElement('button'); db.className='btn danger outline'; db.textContent='Eliminar'; (function(id){db.addEventListener('click',function(){deleteVisit(id);});})(v.id); right.appendChild(db);
...
function renderSummary(){
  var tb=document.querySelector('#summaryTable tbody'); tb.innerHTML='';
  var map={};
  for(var i=0;i<state.visits.length;i++){
    var v=state.visits[i]; var rKey=norm(v.restaurant); var y=yearOf(v.date); var key=rKey+'|'+y;
    if(!map[key])map[key]={restaurantDisplay:v.restaurant,year:y,totalsPP:[],ratings:[],count:0};
    if(v.diners>0)map[key].totalsPP.push(v.total/v.diners); map[key].ratings.push(v.rating||0); map[key].count++;
    if(!map[key].restaurantDisplay)map[key].restaurantDisplay=v.restaurant;
  }
  var rows=[]; for(var k in map){var x=map[k];rows.push({restaurant:x.restaurantDisplay,year:x.year,visits:x.count,avgPP:x.totalsPP.length?avg(x.totalsPP):0,avgRating:x.ratings.length?avg(x.ratings):0});}
  rows.sort(function(a,b){return (b.year-a.year)||a.restaurant.localeCompare(b.restaurant);});
  for(var i=0;i<rows.length;i++){var r=rows[i]; var tr=document.createElement('tr'); tr.innerHTML='<td>'+r.restaurant+'</td><td>'+r.year+'</td><td>'+r.visits+'</td><td>'+fmtEUR(r.avgPP)+'</td><td>⭐ '+fmt2(r.avgRating)+'</td>'; tb.appendChild(tr);}
}
function renderRestaurantFilter(){
  var sel=byId('restaurantFilter'); var map={};
  for(var i=0;i<state.visits.length;i++){var k=norm(state.visits[i].restaurant); if(!map[k])map[k]=state.visits[i].restaurant;}
  var names=[]; for(var k in map)names.push(map[k]); names.sort(function(a,b){return a.localeCompare(b);});
  var current=sel.value; var html='<option value=\"\">— Elige uno —</option>';
  for(var i=0;i<names.length;i++){html+='<option value=\"'+names[i]+'\">'+names[i]+'</option>';} sel.innerHTML=html;
  for(var i=0;i<names.length;i++){if(norm(names[i])===norm(current)){sel.value=names[i];break;}}
}
function renderHistory(){
  var container=byId('historyContainer'); container.innerHTML='';
  var name=byId('restaurantFilter').value;
  if(!name){container.innerHTML='<p class=\"muted\">Elige un restaurante para ver todas sus visitas agrupadas por año.</p>';return;}
  var visits=[]; for(var i=0;i<state.visits.length;i++){if(norm(state.visits[i].restaurant)===norm(name))visits.push(state.visits[i]);}
  visits.sort(function(a,b){return (b.date||'').localeCompare(a.date||'');});
  var byYear={}; for(var i=0;i<visits.length;i++){var y=yearOf(visits[i].date); if(!byYear[y])byYear[y]=[]; byYear[y].push(visits[i]);}
  var years=Object.keys(byYear).map(function(x){return parseInt(x,10);}).sort(function(a,b){return b-a;});
  for(var yi=0;yi<years.length;yi++){
    var y=years[yi]; var arr=byYear[y];
    var avgPP=avg(arr.map(function(v){return v.diners>0?v.total/v.diners:0;}));
    var avgR=avg(arr.map(function(v){return v.rating||0;}));
    var group=document.createElement('div'); group.className='group';
    group.innerHTML='<h4>'+name+' — '+y+' • '+fmtEUR(avgPP)+'/persona • ⭐ '+fmt2(avgR)+' ('+arr.length+' visitas)</h4>';
    var ul=document.createElement('ul'); ul.className='list';
    for(var i2=0;i2<arr.length;i2++){
      var v=arr[i2];
      var li=document.createElement('li');
      li.innerHTML='<div><div><span class=\"badge\">'+v.date+'</span> • <strong class=\"price\">'+fmtEUR(v.total)+'</strong> • '+v.diners+' comensales • '+fmtEUR(v.diners>0?v.total/v.diners:0)+'/persona • ⭐ '+v.rating+'</div>'+(v.notes?'<div class=\"muted\">'+v.notes+'</div>':'')+'</div><div>'+(v.mapsUrl?'<a class=\"btn outline\" href=\"'+v.mapsUrl+'\" target=\"_blank\" rel=\"noopener\">Mapa</a>':'')+'</div>';
...
function computeAggregatedByRestaurantCity(visits){
  ...
  for(var i=0;i<visits.length;i++){
    var v=visits[i]; var city=(v.city||'').trim(); var rKey=norm(v.restaurant); var cKey=norm(city); var key=rKey+'|'+cKey;
    if(!map[key])map[key]={restaurant:v.restaurant,city:city,totalsPP:[],ratings:[],count:0,mapsUrl:'',lastDate:''};
    if(v.diners>0)map[key].totalsPP.push(v.total/v.diners);
    map[key].ratings.push(v.rating||0); map[key].count++;
    if(v.mapsUrl)map[key].mapsUrl=v.mapsUrl;
    if(!map[key].lastDate||(v.date||'')>map[key].lastDate)map[key].lastDate=v.date||'';
  }
  var out=[]; for(var k in map){var x=map[k]; out.push({restaurant:x.restaurant,city:x.city,visits:x.count,avgPP:x.totalsPP.length?avg(x.totalsPP):0,avgRating:x.ratings.length?avg(x.ratings):0,mapsUrl:x.mapsUrl||''});}
  return out;
}
function renderCityFilter(){
  var sel=byId('cityFilter'); var map={}; var arr=state.visits;
  for(var i=0;i<arr.length;i++){var c=(arr[i].city||'').trim(); if(!c)continue; var k=norm(c); if(!map[k])map[k]=c;}
  var names=[]; for(var k in map)names.push(map[k]); names.sort(function(a,b){return a.localeCompare(b);});
  var current=sel.value; var html='<option value=\"\">Todas</option>';
  for(var i=0;i<names.length;i++){html+='<option value=\"'+names[i]+'\">'+names[i]+'</option>';} sel.innerHTML=html;
  for(var i=0;i<names.length;i++){if(norm(names[i])===norm(current)){sel.value=names[i];break;}}
}
function renderExplore(){
  renderCityFilter();
  var info=byId('exploreInfo'); info.textContent='Explora tus restaurantes por ciudad y valoración.';
  var city=byId('cityFilter').value; var minRating=parseInt(byId('minRating').value||'0',10); var sortBy=byId('sortBy').value;
  var rows=computeAggregatedByRestaurantCity(state.visits);
  if(city)rows=rows.filter(function(r){return norm(r.city||'')===norm(city);});
  rows=rows.filter(function(r){return r.avgRating>=minRating;});
  rows.sort(function(a,b){
    switch(sortBy){
      ...
      case'ratingDesc':default:return (b.avgRating-a.avgRating)||(b.visits-a.visits);
    }
  });
  var container=byId('exploreContainer');
  if(!rows.length){container.innerHTML='<p class=\"muted\">No hay resultados con estos filtros.</p>';return;}
  var table=document.createElement('table'); table.innerHTML='<thead><tr><th>Restaurante</th><th>Ciudad</th><th>Visitas</th><th>⭐ media</th><th>€/persona</th><th></th></tr></thead><tbody></tbody>'; table.style.minWidth='640px';
  var tbody=table.querySelector('tbody');
  for(var i=0;i<rows.length;i++){
    var r=rows[i]; var tr=document.createElement('tr');
    tr.innerHTML='<td>'+r.restaurant+'</td><td>'+(r.city||'')+'</td><td>'+r.visits+'</td><td>⭐ '+fmt2(r.avgRating)+'</td><td>'+fmtEUR(r.avgPP)+'</td><td>'+(r.mapsUrl?'<a class=\"btn outline\" href=\"'+r.mapsUrl+'\" target=\"_blank\" rel=\"noopener\">Mapa</a>':'')+'</td>';
    tbody.appendChild(tr);
  }
  container.innerHTML=''; var wrap=document.createElement('div'); wrap.className='table-wrap'; wrap.appendChild(table); container.appendChild(wrap);
}

/* === Restaurant datalist === */
function renderRestaurantDatalist(){
  var dl=document.getElementById('restaurantList'); if(!dl)return;
  var map={},names=[];
  for(var i=0;i<state.visits.length;i++){var k=norm(state.visits[i].restaurant); if(!map[k])map[k]=state.visits[i].restaurant;}
  for(var k in map)names.push(map[k]); names.sort(function(a,b){return a.localeCompare(b);});
  var html=''; for(var i=0;i<names.length;i++){html+='<option value=\"'+names[i]+'\"></option>';} dl.innerHTML=html;
}

/* === Delete === */
function deleteVisit(id){
  var idx=-1; for(var i=0;i<state.visits.length;i++){if(state.visits[i].id===id){idx=i;break;}}
  if(idx===-1)return;
  if(confirm('¿Seguro que quieres eliminar esta visita?')){
    var removed=state.visits[idx]; state.visits.splice(idx,1); save();
    if(ensureSupa()){ try{ getCurrentUser().then(function(u){ if(u){ syncDeleteVisit(removed.id).catch(function(){}); } }); }catch(e){} }
    renderAll();
  }
}
...
function renderRecentFriendsUI(){
  var dl=byId('friendUserList'); if(dl){var list=getRecentFriends();dl.innerHTML=list.map(function(u){return '<option value=\"'+u+'\"></option>';}).join('');}
  var chips=byId('friendRecentChips');
  if(chips){
    var list=getRecentFriends();
    chips.innerHTML=list.length?list.slice(0,10).map(function(u){return '<button class=\"chip\" data-u=\"'+u+'\">'+u+'</button>';}).join(' '):'<span class=\"muted\">Sin recientes</span>';
    chips.onclick=function(e){var b=e.target&&e.target.closest?e.target.closest('button[data-u]'):null;if(!b)return;byId('friendUsername').value=b.getAttribute('data-u');byId('loadFriendBtn').click();};
  }
}

/* === Supabase (profiles + summaries; sessions persistentes) === */
...
var supa=null;
function ensureSupa(){if(!SUPA.url||!SUPA.anon)return null;try{if(!supa){supa=window.supabase.createClient(SUPA.url,SUPA.anon,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});}return supa;}catch(e){return null;}}
...
/* === VISITS sync opcional (mantiene local si no hay sesión) === */
...
function initFriends(){
  ...
  var lf=byId('loadFriendBtn'); if(lf)lf.addEventListener('click',function(){var uname=byId('friendUsername').value.trim();if(!uname){alert('Introduce el usuario de tu amigo');return;}fetchFriendSummary(uname).then(function(r){var items=(r&&r.data&&r.data.items)||[];renderFriendSummary(items);addRecentFriend(uname);}).catch(function(err){alert(err.message||err);});});
  renderRecentFriendsUI(); updateAccountInfo();
}
...
function renderFriendSummary(items){
  var cont=byId('friendSummaryContainer'); cont.innerHTML='';
  if(!items||!items.length){cont.innerHTML='<p class=\"muted\">Nada que mostrar.</p>';return;}
  var table=document.createElement('table'); table.innerHTML='<thead><tr><th>Restaurante</th><th>Ciudad</th><th>Visitas</th><th>⭐ media</th><th>€/persona</th><th></th></tr></thead><tbody></tbody>';
  var tb=table.querySelector('tbody');
  for(var i=0;i<items.length;i++){
    var r=items[i]; var tr=document.createElement('tr');
    tr.innerHTML='<td>'+r.restaurant+'</td><td>'+(r.city||'')+'</td><td>'+r.visits+'</td><td>⭐ '+fmt2(r.avgRating||0)+'</td><td>'+fmtEUR(r.avgPP||0)+'</td><td>'+(r.mapsUrl?'<a class=\"btn outline\" href=\"'+r.mapsUrl+'\" target=\"_blank\" rel=\"noopener\">Mapa</a>':'')+'</td>';
    tb.appendChild(tr);
  }
  var wrap=document.createElement('div'); wrap.className='table-wrap'; wrap.appendChild(table); cont.appendChild(wrap);
}
